...
__global__ void doubleMemset(double* ptr, unsigned int length, double value)
{
...
void* dgemmSweep(void* devID)
{
    ...
    for (int curIter = 0; curIter < iterations; curIter++)
    {
        for (i = 128; i < iterSize; i+= speedSetting)
        {
            ...
            printf("Device %d: i = %d\n",device, i);
            double* c_h = (double*)malloc(sizeof(double) * i*i);
            if (!c_h)
            {
            ...
            if (cudaMalloc((void**)&A, i*i*sizeof(double)) != cudaSuccess)
            {
            ...
            if (cudaMalloc((void**)&B, i*i*sizeof(double)) != cudaSuccess)
            {
...
for (int i = 0; i < testedDevices; i++)
{
    ...
    {
        printf("ERROR: Failed with device %d. dgemmSweep FAILED.\n", terminatingDevice);
        exit(1);
    }
}
printf("dgemmSweep PASSED.\n");
}
